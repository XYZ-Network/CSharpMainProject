...
{
    public partial class CustomUnitPath : BaseUnitPath
    {
        private int[] dx = { -1, -1, 0, 1, 1, 1, 0, -1};
        private int[] dy = { 0, 1, 1, 1, 0, -1, -1, -1 };

        public CustomUnitPath(IReadOnlyRuntimeModel runtimeModel, Vector2Int startPoint, Vector2Int endPoint) : base(runtimeModel, startPoint, endPoint)
        {
        }
        ...
        protected override void Calculate()
        {
            ...
            while (openList.Count < runtimeModel.RoMap.Width * runtimeModel.RoMap.Height)
            {
                ...
                {
                    int newX = currentNode.coordinates.x + dx[i];
                    int newY = currentNode.coordinates.y + dy[i];
